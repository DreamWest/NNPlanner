...
from networks.policy_simple_avg_pool import NNPlanner, get_init_hidden_state
...
def parse_args():
    ...
    parser.add_argument("--use-latent-loss", action="store_true")
    ...
    parser.add_argument("--use-global-frame", action="store_true")
    parser.add_argument("--norm-goal-by-horizon", action="store_true")
    parser.add_argument("--use-attitude", action="store_true")
    parser.add_argument("--use-angvel", action="store_true")
    return parser.parse_args()


def main(args):
    ws_dir = os.path.join(os.path.expanduser('~'), "catkin_ws_fastplanner_unity")
    work_dir = os.path.join(ws_dir, "results", args.work_dir, args.exp_name)
    ...
    replay_buffer = ReplayBuffer(dataset_path, device, n_steps=args.n_steps, mask_file=None, use_global_frame=args.use_global_frame,
                                 norm_goal_by_horizon=args.norm_goal_by_horizon, use_attitude=args.use_attitude, use_angvel=args.use_angvel)
    
    print("use attitude: {}, use angvel: {}".format(args.use_attitude, args.use_angvel))
    state_dim, action_dim = replay_buffer.compute_state_action_dim()

    nn_planner = NNPlanner(state_dim, action_dim, args.feature_dim, args.hidden_dim).to(device)
    ...
    for i in tqdm(range(args.num_pretrained_epochs)):
        ...
        for batch in replay_buffer.sample(args.batch_size, "train"):
            ...
            if args.use_latent_loss:
                train_hv = train_hv.view(train_hv.size(0), train_hv.size(1), train_hv.size(2), -1)
                train_ae_loss = F.mse_loss(train_recons, train_batch_depths) + 0.5 * train_hv.pow(2).sum(-1).mean()
            else:
                train_ae_loss = F.mse_loss(train_recons, train_batch_depths)
        ...
        with torch.no_grad():
            for batch in replay_buffer.sample(args.batch_size, "val"):
                ...
                if args.use_latent_loss:
                    val_hv = val_hv.view(val_hv.size(0), val_hv.size(1), val_hv.size(2), -1)
                    val_ae_loss = F.mse_loss(val_recons, val_batch_depths) + 0.5 * val_hv.pow(2).sum(-1).mean()
                else:
                    val_ae_loss = F.mse_loss(val_recons, val_batch_depths)
